...
def intersection(start1, end1, start2, end2):
    ...
    if start1.x == start2.x or end1.x == start2.x or start1.y == start2.y or end1.y == start2.y or start1.x == end2.x or end1.x == end2.x or start1.y == end2.y or end1.y == end2.y:
        return True
...
class Boolet(pg.sprite.Sprite):
    ...
    def check_cells(self, movevector):
        ...
        if collided_cell:
            ...
            # Верх
            print(topleft, topright, linestart, lineend)
            print(movevector)
            if intersection(topleft, topright, linestart, lineend):
                ...
                if movevector[0] > 0:
                    ...
                    self.angle = self.angle - 90
                self.rect.top = collided_cell.rect.top - 20
            # Низ
            if intersection(bottomleft, bottomright, linestart, lineend):
                self.angle = self.angle - 90
                self.rect.bottom = collided_cell.rect.bottom + 30
            # Лево
            if intersection(topleft, bottomleft, linestart, lineend):
                self.angle = -self.angle
                self.rect.left = collided_cell.rect.left - 10
            # Право
            if intersection(topright, bottomright, linestart, lineend):
                self.angle = -self.angle
                self.rect.right = collided_cell.rect.right + 10
