...
def slicingAnimations(animationName, xW, yH, quality, long):
    imageAnimation = pg.image.load("animations/{}".format(animationName))
    imageSlise = imageAnimation
    listAnimation = []
    for i in range(quality):
        spriteRect = pg.Rect(1*quality, 0, xW, yH)
        spriteImage = imageAnimation.subsurface(spriteRect)
        imageSlise = imageAnimation
        if long != 0:
            for i in range(long-1):
                listAnimation.append(spriteImage)
...
class Explosion(pg.sprite.Sprite):
    ...
    def __init__(self, thing, center, power, duration=0):
        ...
        self.listAnimation = slicingAnimations("boom.png", 32, 32, 10, 2)
        self.thing.kill()
    ...
    def update(self, events):
        ...
        if self.duration == 19:
            ...
            pg.blit(self.listAnimation[self.duration], self.center)
            self.rect = self.image.get_rect(center=self.rect.center)
            self.duration += 1
        counter += 1
        #self.duration -= 1
